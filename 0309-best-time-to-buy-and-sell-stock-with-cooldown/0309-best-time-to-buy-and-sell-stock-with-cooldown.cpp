...
public:
    int maxProfit(vector<int>& prices) {
        int n = prices.size();
        vector<int> Extraahead(2,0),ahead(2,0), curr(2,0);
        ...
        for(int idx=n-1;idx>=0;idx--)
        {
            for(int buy=1;buy>=0;buy--)
            {
                ...
                {
                    maxProfit = max(prices[idx] + Extraahead[1],ahead[0]); // 1st instance is of selling which leads to buy=1 since you are allowed to buy after selling while 2nd instance is of continuing to other day without selling the stock in order to look for better profit.
                }
